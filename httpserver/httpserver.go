...
func New(log Logger, name string, port string, h http.Handler, opts ...ServerOption) *Server {
	var s Server
	return new_(&s, log, name, port, h, opts...)
}

// internal 'function outlining' to promote Server to the callers stack
func new_(s *Server, log Logger, name string, port string, h http.Handler, opts ...ServerOption) *Server {
